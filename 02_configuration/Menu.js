...
function onOpen() {
  ...
  ui.createMenu("🏭 Usine")
    ...
    .addSubMenu(
      ui
        .createMenu("Déploiement")
        .addItem("Générer le lien du test...", "lancerDeploiement_UI")
    )
    ...
    .addSubMenu(
      ui
        ...
        .addItem(
          "Vérifier les en-têtes (CONFIG, BDD, TEMPLATE)",
          "validateAllHeaders"
        )
    )
    .addSeparator()
    .addSubMenu(
      ui
        ...
        .addItem(
          "Générer la fiche de test (imprimable)...",
          "showPrintableSheetUI"
        )
    )
    .addToUi();
}
...
function getInitialData() {
  try {
    ...
    headers.forEach((header, i) => {
      if (header)
        optionsMap[header] = optionsData.map((row) => row[i]).filter(String);
    });
    ...
    try {
      ...
      if (metaSheet) {
        const metaData = metaSheet
          .getRange(2, 1, metaSheet.getLastRow() - 1, 3)
          .getValues();
        availableMetaBlocks = metaData
...
function getQuestionCountForTestType(typeTest) {
  ...
  try {
    ...
    if (systemIds && systemIds.ID_BDD) {
      const bdd = SpreadsheetApp.openById(systemIds.ID_BDD);
      const questionSheet = bdd
        .getSheets()
        .find((s) => s.getName().startsWith("Questions_" + typeTest));
      return questionSheet ? questionSheet.getLastRow() - 1 : 0;
    }
    return 0;
  } catch (err) {
    Logger.log(
      "Erreur getQuestionCountForTestType pour " + typeTest + ": " + err.message
    );
    return 0;
  }
}
...
function processNewTestConfiguration(formObject) {
  try {
    ...
    const headers = paramsSheet
      .getRange(1, 1, 1, paramsSheet.getLastColumn())
      .getValues()[0];
    ...
    dataRow["Limite_Lignes_A_Traiter"] = getQuestionCountForTestType(
      formObject.type
    );
    dataRow["Blocs_Meta_A_Inclure"] = formObject.blocsMeta
      ? formObject.blocsMeta.join(",")
      : "";

    if (String(formObject.repondantContenu).includes("Niveau1"))
      dataRow["ID_Gabarit_Email_Repondant"] = "RESULTATS_N1";
    else if (String(formObject.repondantContenu).includes("Niveau2"))
      dataRow["ID_Gabarit_Email_Repondant"] = "RESULTATS_N2";
    else if (String(formObject.repondantContenu).includes("Niveau3"))
      dataRow["ID_Gabarit_Email_Repondant"] = "RESULTATS_N3";

    // Emails
    dataRow["Repondant_Email_Actif"] = formObject.repondantActif
      ? "Oui"
      : "Non";
    ...
    dataRow["Formateur_Email_Actif"] = formObject.formateurActif
      ? "Oui"
      : "Non";
    ...
    const nouvelleLigne = headers.map((header) =>
...
function showEditSidebar_UI() {
  ...
  const response = ui.prompt(
    "Modifier un Test",
    "Veuillez entrer le numéro de la ligne à modifier :",
...
function showEditSidebar(rowIndex) {
  ...
  const html = template
    .evaluate()
    .setTitle("Édition du Test (Ligne " + rowIndex + ")")
    .setWidth(600);
  SpreadsheetApp.getUi().showSidebar(html);
}
...
function getTestDataForEdit(rowIndex) {
  const sheet = SpreadsheetApp.openById(ID_FEUILLE_CONFIG).getSheetByName(
    "Paramètres Généraux"
  );
...
function showDuplicateUI() {
  const ui = SpreadsheetApp.getUi();
  const response = ui.prompt(
    "Dupliquer une Configuration de Test",
    "Veuillez entrer le numéro de la ligne à dupliquer :",
    ui.ButtonSet.OK_CANCEL
  );
  if (response.getSelectedButton() == ui.Button.OK) {
    const rowIndex = parseInt(response.getResponseText());
    if (!isNaN(rowIndex) && rowIndex > 1) {
      try {
        ...
        ui.alert(
          "Succès",
          `La ligne ${rowIndex} a été dupliquée à la fin de la feuille (nouvelle ligne : ${newRowIndex}).`,
          ui.ButtonSet.OK
        );
      } catch (e) {
...
function duplicateTestConfiguration(rowIndex) {
  ...
  const fieldsToClear = [
    "Id_Unique",
    "Nom_Fichier_Complet",
    "Lien_Formulaire_Public",
    "ID_Formulaire_Cible",
    "ID_Sheet_Cible",
    "Accès Direct Formulaire",
  ];
  const shortUrlHeader = headers.find(
    (h) => h && h.toLowerCase().includes("raccourci")
  );
  ...
  const newValues = headers.map((header) => {
    ...
    if (header === "Titre_Formulaire_Utilisateur")
      return sourceValue + " (Copie)";
    return sourceValue;
  });
...
function showPrintableSheetUI() {
  if (ID_MODELE_FICHE_TEST === "METTEZ_ICI_L_ID_DE_VOTRE_MODELE_GOOGLE_DOC") {
    SpreadsheetApp.getUi().alert(
      ...
      "Veuillez d'abord renseigner l'ID de votre modèle Google Doc.",
    ...
    if (!isNaN(rowIndex) && rowIndex > 1) {
      try {
        ...
        const htmlOutput = HtmlService.createHtmlOutput(
          `<p>La fiche de test a été générée.</p><a href="${fileUrl}" target="_blank">Ouvrir le document</a>`
        )
          .setWidth(300)
          .setHeight(100);
...
function generatePrintableSheet(rowIndex) {
  ...
  const testTitle =
    dataForFusion["Titre_Formulaire_Utilisateur"] || "Fiche de Test";
...
function getSystemIds() {
  ...
  if (!idSheet) {
    throw new Error("L'onglet 'sys_ID_Fichiers' est introuvable.");
  }
