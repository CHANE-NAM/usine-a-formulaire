...
// ID du modèle pour la fiche de test (catalogue)
...
// --- SECTION 2 : FONCTIONS POUR LA CRÉATION D'UN NOUVEAU TEST ---
...
// --- SECTION 3 : FONCTIONS POUR L'ÉDITION D'UN TEST EXISTANT ---
...
// --- SECTION 5 : FONCTIONS POUR LA GÉNÉRATION DE DOCUMENTS ---

function showPrintableSheetUI() {
  if (ID_MODELE_FICHE_TEST === "METTEZ_ICI_L_ID_DE_VOTRE_MODELE_GOOGLE_DOC") {
    SpreadsheetApp.getUi().alert("Configuration requise", "Veuillez d'abord renseigner l'ID de votre modèle Google Doc dans le script Menu.gs (variable ID_MODELE_FICHE_TEST).", SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  const ui = SpreadsheetApp.getUi();
  const response = ui.prompt('Générer une Fiche de Test', 'Veuillez entrer le numéro de la ligne à utiliser pour générer le document :', ui.ButtonSet.OK_CANCEL);
  if (response.getSelectedButton() == ui.Button.OK) {
    const rowIndex = parseInt(response.getResponseText());
    if (!isNaN(rowIndex) && rowIndex > 1) {
      try {
        const fileUrl = generatePrintableSheet(rowIndex);
        const htmlOutput = HtmlService.createHtmlOutput(`<p>La fiche de test a été générée avec succès.</p><a href="${fileUrl}" target="_blank">Cliquez ici pour ouvrir le document</a>`).setWidth(300).setHeight(100);
...
function generatePrintableSheet(rowIndex) {
  const sheet = SpreadsheetApp.openById(ID_FEUILLE_CONFIG).getSheetByName("Paramètres Généraux");
  if (rowIndex > sheet.getLastRow()) throw new Error("La ligne spécifiée n'existe pas.");
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  const values = sheet.getRange(rowIndex, 1, 1, sheet.getLastColumn()).getValues()[0];
  const dataForFusion = {};
  headers.forEach((header, i) => {
