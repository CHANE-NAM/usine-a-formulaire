 ...
 * == VERSION : 4.0 - Intégration d'un système de débogage activable.
...
function assemblerEtEnvoyerEmailUniversel(config, reponse, resultats, langueCible, optionsSurcharge = {}, kitSpreadsheet) {
  ...
  _log_mail(true, '--- DÉBUT ASSEMBLAGE EMAIL ---');
  ...
  if (briquesDeContenu.length > 0) {
    _log_mail(true, 'Première brique trouvée (pour vérification) :', briquesDeContenu[0]);
  } else {
    _log_mail(true, 'AVERTISSEMENT : Aucune brique de contenu trouvée. L\'e-mail sera probablement vide.');
  }
