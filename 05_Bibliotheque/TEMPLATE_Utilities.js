 ...
 * == VERSION : 12.0 - Intégration d'une gestion d'erreur stricte.
...
function _tryReadKeyValueOrHorizontalConfig(fileId, possibleSheetNames, kitSpreadsheet) {
  try {
    ...
    // MODIFICATION CRUCIALE : Le fallback permissif est remplacé par une erreur stricte.
    if (!targetRow) {
      throw new Error(`Configuration introuvable pour le kit ID "${kitId}" dans l'onglet "${sh.getName()}". Vérifiez la colonne 'ID_Sheet_Cible'.`);
    }
    ...
    // Propage l'erreur pour qu'elle soit visible dans les logs d'exécution.
    throw e;
  }
}
