...
# === 1) AI-friendly ===
# CORRECTION: correspond à "scripts_" (1 underscore), pas "scripts__"
...
try {
  ...
  $gitLog = git log --oneline -n 12 2>$null
} finally {
  Pop-Location
}
...
if ($aiFiles.Count -gt 0) {
  ...
  [void]$sbEtat.AppendLine('_Aucun document AI généré (pas de scripts_*.txt trouvés)._')
}
